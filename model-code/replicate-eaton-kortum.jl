...
dftest = DataFrame(CSV.File("./make-trade-share/tradeshare-df-2011.csv"))

dftest[!,"trade"] = log.(dftest[!,"norm_tradeshare"] )

filter!(row -> ~(row.norm_tradeshare ≈ 1.0), dftest);

filter!(row -> ~(row.norm_tradeshare ≈ 0.0), dftest);

grvdata = gravity(dftest, display = true);
