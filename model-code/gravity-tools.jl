    ...
    for importer = 1:Ncntry
        ...
        for exporter = 1:Ncntry
            ...
            if exporter != importer
                ...
                # println("importer: ", importer, " exporter: ", exporter)
                ...
                d[importer, exporter] =(distance_effect * border_effect  * asym_effect)
...
function make_trademodel_dataframe(πshares, gravity_params)
    # function to make the trade model dataframe

    # important that the πshares are setup so the importer is the row
    # and the exporter is the column so its a sum accross the columns that
    # adds up to one

    @unpack dfcntryfix, Ncntry = gravity_params
    
    df = DataFrame(exporter = String[], importer=String[], tradeshare=Float64[], norm_tradeshare=Float64[], 
        tradedata = Float64[], norm_tradedata=Float64[])
    ...
    for importer = 1:Ncntry
        ...
        for exporter = 1:Ncntry
            ...
            # println("importer: ", importer, " exporter: ", exporter)
                ...
                exporter_name = foo[get_exporter, :].iso_o[1]

                importer_name = foo[get_exporter, :].iso_d[1]

                push!(df, (exporter_name, importer_name, πshares[importer, exporter],
                    πshares[importer, exporter] / πshares[importer, importer],
                    foo[get_exporter, :].tradeshare[1], foo[get_exporter, :].norm_tradeshare[1]))

            end

            # else

            #     exporter_name = foo[get_exporter, :].iso_d[1]

            #     importer_name = foo[get_exporter, :].iso_o[1]

            # end

            # # Append the data to the DataFrame
            # push!(df, (exporter_name, importer_name, πshares[importer, exporter],
            # πshares[importer, exporter] / tradeshare[importer, importer]))
        end
    end
